...
const deleteLoan = async (req, res) => {
  try {
    const loan = await Loan.findById(req.params.id)
    if (!loan) return res.status(404).json({ success: false, message: 'Loan not found', errors: {} })
    await LoanPayment.deleteMany({ loan: loan._id })
...
const cancelLoan = async (req, res) => {
  try {
    ...
    const loan = await Loan.findById(req.params.id)
    if (!loan) return res.status(404).json({ success: false, message: 'Loan not found', errors: {} })
    if (loan.status === 'cancelled') return res.status(422).json({ success: false, message: 'Loan is already cancelled', errors: {} })
...
const addPayment = async (req, res) => {
  try {
    ...
    if (!loan) return res.status(404).json({ success: false, message: 'Loan not found', errors: {} })
    if (loan.status === 'cancelled') return res.status(422).json({ success: false, message: 'Cannot add payment to a cancelled loan', errors: {} })
    if (amount > loan.remainingAmount) return res.status(422).json({ success: false, message: 'Payment amount cannot exceed remaining amount', errors: {} })
...
const removePayment = async (req, res) => {
  try {
    ...
    if (!loan) return res.status(404).json({ success: false, message: 'Loan not found', errors: {} })
    if (loan.status === 'cancelled') return res.status(422).json({ success: false, message: 'Cannot remove payment from a cancelled loan', errors: {} })
